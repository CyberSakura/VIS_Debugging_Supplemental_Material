...
| 6 | Knowledge Gap | The participant's limited understanding of a language, tool, or debugging mechanism impedes interpretation of the problem, often requiring extra effort to make sense of errors. | "As a non-CS person, I have to put in extra effort to understand and fix it." / "Sometimes the error messages aren't very obvious." |
...
| 2 | Manual Effort | The participant completes debugging steps through their own analysis or by hand — examining code line by line, observing variable values, manually modifying scripts, or reproducing the problem. <br><br> This also includes code-level operations used to validate or confirm suspected errors, such as inserting print statements, adjusting parameters, and partial reruns. Encompasses inspection, hypothesis formation, controlled manipulation, and iterative re-execution; emphasizes human-led reasoning and experimentation. | "I manually checked variable types." / "I looked at the functions being called and tested them outside the wrapper." / <br><br> "I ran some code to monitor memory usage." / "Printed file path to check configuration." |
...
| 4 | External Tool Assistance | Using built-in or external debugging tools (e.g., IDE debugger, Python toolbox, Spark UI) to visualize and trace program flow or monitor resource usage. | "I used the IDE debugger in Python or in VS Code." / "The Spark Web UI lets me drill down into text-level details." |
| 5 | Task Decomposition / Prompting | Breaking large or complex tasks into small, manageable steps, or progressively guiding the AI or oneself through refined questions (prompting). Reflects systematic thinking and task decomposition. | "Broke task into small subtasks for AI." / "Prompted step by step." |
| 6 | Mental Confirmation | After experimenting or reviewing evidence, the participant verbalizes their understanding or causal reasoning — "what's going on" and "why." Reflects internal sense-making closure. | "After these steps, I already have an idea of what's going on." / "Now I know what caused it." |
| 7 | Temporary Workaround / Code Bypass | Skipping or rewriting problematic code temporarily to keep progress when direct debugging is too time-consuming. | "If a section doesn't work, I'll comment it out and still run the rest." / "I rewrote B to keep A→C running." |
| 8 | Strategic Regulation / Goal Prioritization | Consciously regulating debugging effort — knowing when to stop, switch direction, or focus on execution first. Reflects meta-cognitive awareness of time and value. | "Knowing when to switch directions is very important." / "First, my priority is to make the code run." |
...
| 2 | Log / Error Trace | Consulting system output information (e.g., logs, error messages, or stack traces) for clues to the cause of a failure. Usually involves reading system-generated messages (e.g., error message, kernel log). | "I read the kernel's log file." / "Looked at error message for clues." |
| 3 | Variable / Object Info | Checking variable, object, or data-structure information at runtime to verify values, types, sizes, distributions, or missing entries. Reflects exploration of the program's internal state. | "Checked whether it returns the correct data type or data structure." / "I look at the distribution and check for outliers and missing values." |
...
| 7 | Code Comments / Internal Documentation | Reliance on (or expectation of) comments, docstrings, or internal documentation to understand code logic, functionality, or execution flow. Unlike runtime output (e.g., logs), this is static, human-readable explanation written by developers; captures attention to comment quality and internal interpretability when reading unfamiliar code. | "Hopefully, the code has good comments so you can follow along." / "I read through the docstrings to understand what each function does." |
| 8 | Mental Clue / Hypothesis Formation | The participant forms clue-based hypotheses or inferences about the problem based on their own task, experience, and knowledge, establishing initial clues that are then verified through subsequent steps. | "That was the first clue." |
